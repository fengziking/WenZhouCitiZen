...
+ (int)compareOneDay:(NSDate *)oneDay withAnotherDay:(NSDate *)anotherDay
{
    ...
    KNSLog(@"oneDay : %@, anotherDay : %@", oneDay, anotherDay);
...
//获取网络时间
+ (NSString *)getInternetNetWorkDateString
{
...
+ (NSDate *)getDateInternetNet {
    ...
    NSString *outputStr = [outputFormatter stringFromDate:inputDate];
    NSDate *outputDate = [outputFormatter dateFromString:outputStr];
    return outputDate;
}

+ (NSInteger)compareFormater:(NSString *)formater Date:(NSString*)aDate withDate:(NSString*)bDate
{
    NSInteger aa = 0;
//    NSString *format = [NSString isNull:format] ? @"yyyy-MM-dd HH:mm:ss" : formater;
