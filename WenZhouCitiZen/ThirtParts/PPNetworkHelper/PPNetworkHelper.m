...
//针对项目存放dic 存放任务task和reCode
static NSMutableArray *_allTaskDicArr;
#pragma mark - 开始监听网络
...
+ (BOOL)isNetwork
{
    KNSLog(@"%d",[AFNetworkReachabilityManager sharedManager].reachable);
...
+ (NSDictionary *)getTaskDic:(NSDictionary *)parameters task:(NSURLSessionTask *)task {
    NSMutableDictionary *mDic = @{}.mutableCopy;
   // [mDic setObject:parameters[kReqCode] forKey:kReqCode];
   // [mDic setObject:task forKey:@"task"];
...
{
    //读取缓存
    NSDictionary *cacheDic = @{kReqCode:parameters[kReqCode]};
    responseCache ? responseCache([PPNetworkCache httpCacheForURL:URL parameters:cacheDic]) : nil;
...
//        [[self allSessionTask] removeObject:task];
        [[self allTaskDicArr] removeObject:[self getTaskDic:parameters task:task]];
        
        success ? success(responseObject) : nil;
        //对数据进行异步缓存
        responseCache ? [PPNetworkCache setHttpCache:responseObject URL:URL parameters:cacheDic] : nil;
        KNSLog(@"\n<<-----------请求返回内容--------------\n 链接和参数 == %@%@\n 内容 == %@\n",URL , parameters, [self jsonToString:responseObject]);
...
//    sessionTask ? [[self allSessionTask] addObject:sessionTask] : nil ;
    sessionTask ? [[self allTaskDicArr] addObject:[self getTaskDic:parameters[kReqCode] task:sessionTask]] : nil;
...
//    NSDictionary *cacheDic = @{kReqCode:parameters[kReqCode]};
//    if (responseCache) {
//        id cache = [PPNetworkCache httpCacheForURL:URL parameters:cacheDic];
//        if (cache ) {
//            responseCache(cache);
//        }
//    }
...
//    sessionTask ? [[self allSessionTask] addObject:sessionTask] : nil ;
    ...
    sessionTask ? [[self allTaskDicArr] addObject:[self getTaskDic:parameters task:sessionTask]] : nil;
...
#pragma mark -- 创建项目扩展
+ (NSMutableArray *)allTaskDicArr {
    if (!_allTaskDicArr) {
        _allTaskDicArr = [[NSMutableArray alloc] init];
    }
    return _allTaskDicArr;
}
@end
